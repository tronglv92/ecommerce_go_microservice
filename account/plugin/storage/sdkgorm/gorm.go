...
import (
	...
	"github.com/tronglv92/accounts/plugin/storage/sdkgorm/gormdialects"
...
func (gdb *gormDB) InitFlags() {
	...
	flag.StringVar(&gdb.UriReadOnly, prefix+"gorm-db-uri-readonly", "", "Gorm database slave connection-string.")
...
func (gdb *gormDB) Configure() error {
	...
	gdb.logger.Info("Connect to Gorm DB at ", gdb.UriReadOnly, " ...")
...
// func (gdb *gormDB) getReplicate(dbType string) []gorm.Dialector {
// 	switch strings.ToLower(dbType) {
// 	case "mysql":
// 		return []gorm.Dialector{
// 			mysql.Open(gdb.UriReadOnly),
// 		}
// 	case "postgres":
// 		return []gorm.Dialector{
// 			postgres.Open(gdb.UriReadOnly),
// 		}
// 	case "sqlite":
// 		return []gorm.Dialector{
// 			sqlite.Open(gdb.UriReadOnly),
// 		}
// 	case "mssql":
// 		return []gorm.Dialector{
// 			mysql.Open(gdb.UriReadOnly),
...
func (gdb *gormDB) getDBConn(t GormDBType) (dbConn *gorm.DB, err error) {
	...
	uriReadOnly := strings.Split(gdb.UriReadOnly, ",")
	switch t {
	case GormDBTypeMySQL:
		return gormdialects.MySqlDB(gdb.Uri, uriReadOnly)
	case GormDBTypePostgres:
		return gormdialects.PostgresDB(gdb.Uri, uriReadOnly)
	case GormDBTypeSQLite:
		return gormdialects.SQLiteDB(gdb.Uri, uriReadOnly)
	case GormDBTypeMSSQL:
		return gormdialects.MSSqlDB(gdb.Uri, uriReadOnly)
	}
